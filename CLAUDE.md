...
**TOTP (`totp.py`)** — pure functions over `Account`. `normalize_secret` is the single choke point for Base32 validation and is called by both the manual-entry dialog and `parse_otpauth_uri`; any new entry path must go through it. `current_code` / `seconds_remaining` are called every tick from the UI and must stay cheap.
